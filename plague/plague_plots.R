...
ggsave(plot = plot1, "plague_seasonality.pdf", width = 8, height = 5)
...
# function defined from start of large epidemic wave (day 53, lasts for 66 days)
t <- seq(1, 66, by = 0.001)
...
ggsave(plot = plot2, "plague_interventions_vector.pdf", width = 8, height = 5)


# delay for detection compartment
...
f.det_vals %>%
  ...
  annotate("text", x = delay, y = -0.09, label = TeX("$\\tau$"), color = "orange", size = 6) +  
  coord_cartesian(ylim = c(0,1), clip = "off") +
  theme_gray(base_size=15) -> plot3

ggsave(plot = plot3, "plague_delay_detection.pdf", width = 8, height = 5)



## Fitted curve

out_df <- data.frame(out_fit) #out_fit result from plague_fit R file
...
df_plague_relevant %>%
  ...
  theme_gray(base_size=15) +
  geom_line(aes(x=time, y=out_df$I1), size=1) -> plot4

ggsave(plot = plot4, "plague_fitted_curve.pdf", width = 6, height = 5)